...
with col1:
        st.header('MetaStackVis: Visually-Assisted Performance Evaluation of Metamodels in Stacking Ensemble Learning')
...
with st.expander('Initial information'):
        st.markdown(
        '''
                Stacking methods (or stacked generalizations) refer to a group of ensemble learning methods where several base models ("weak learners") are trained and combined into a metamodel with improved predictive power. Stacked generalization can reduce the bias and decrease the generalization error when compared to the use of single learning algorithms. This work is based on data from the visual analytics system, called StackGenVis, developed by the ISOVIS research group at LNU and requires input data from that tool. Please find more information about the tool [here](https://github.com/angeloschatzimparmpas/StackGenVis). 

                This tool can be considered a further development of the original StackGenVis tool, as it provides the end user with the possibility to further investigate the impact of different clusters of based models on the overall performance of the metamodel in stacked generalization using HDBSCAN cluster comparison, UMAP, and coverage analysis.

                The tool requires a general understanding of the concepts of stacked generalization and the use of the original StackGenVis tool. The tool is developed in Python using the Streamlit library. MetaStackVis cannot be used as a standalone application as it depends on data generated by StackGenVis.
...
with st.expander('Usage information'):
        ...
        '''
                ...
                6. Plot the coverage analysis of all metamodels for a specific cluster
                ---
...
with st.expander('Credits and author of the tool'):
                st.markdown(
        '''       
                This work is done as part of a bachelor's thesis at CS LNU in 2022.
                ...
                ---
                ***Linnaeus university, Faculty of Technology - 2022***
