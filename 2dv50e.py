...
def umap_plot(df_umap):
        ...
        fig.update_layout(width=plotly_size, height=plotly_size)
        # add tooltip
...
if uploaded_probabilities is not None and uploaded_model is not None and uploaded_target is not None \
        ...
        if df_probabilities.isnull().values.any() or df_model.isnull().values.any() or df_target.isnull().values.any() \
                or df_source.isnull().values.any():
                st.write('There are some missing values in some of the dataframes, please correct datasets and re-upload the files')
                st.write(f'The number of missing values in model probabilities dataset is {df_probabilities.isnull().sum()}')
                st.write(f'The number of missing values in top models dataset is {df_model.isnull().sum()}')
                st.write(f'The number of missing values in target dataset is {df_target.isnull().sum()}')
                st.write(f'The number of missing values in original dataset is {df_source.isnull().sum()}')
